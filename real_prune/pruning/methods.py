...
def weight_prune(model, pruning_perc):
    ...
    threshold = []
    for p in model.parameters():
        if len(p.data.size()) != 1 and ind<=2:
            tmp_weights = list(p.cpu().data.abs().clone().numpy().flatten())
            tmp_shreshold= np.percentile(np.array(tmp_weights), pruning_perc)
            threshold.append(tmp_shreshold)
            ind+=1
    ...
    masks = []
    for p in model.parameters():
        if len(p.data.size()) != 1 and count<=2:
            pruned_inds = p.data.abs() > threshold[count]
            masks.append(pruned_inds.float())
            count+=1
    return masks
