...
def prune_vgg16_conv_layer(model, layer_index, filter_list):
	...
	if not next_conv is None:
		next_new_conv = \
			torch.nn.Conv2d(in_channels = next_conv.in_channels - len(filter_list),\
	...
	if not next_conv is None:
		...
		params_per_input_channel = old_linear_layer.in_features / conv.out_channels
	 	...
	 	new_weights[:, : filter_list * params_per_input_channel] = \
	 		old_weights[:, : filter_list * params_per_input_channel]
	 	new_weights[:, filter_list * params_per_input_channel :] = \
	 		old_weights[:, (filter_list + 1) * params_per_input_channel :]
